...
def encode_password(password):
    byte_str = bytes.fromhex(password[2:])  # Decode strings into byte strings
    utf8_str = byte_str.decode('utf-8')  # Decode byte strings into UTF-8 encoded strings
    new_password = utf8_str.encode('utf-8').replace(b'$', b'\x24')
    # Encode the string into a UTF-8 byte string and replace the "$" character with "\x24"
    return new_password
